...
\begin{essyntax}
    ...
    tabulated bond \var{filename} \opt{\var{breakable}}
...
\begin{pysyntax}
  \object{
   ...
   (type=\arg{"distance" or "angle" or "dihedral"} ,
    filename = \arg{str},
    breakable = \arg{int})
  }
\end{pysyntax}
...
The \texttt{breakable} parameter controls the behavior for a distance based interaction, when the particle distance is larger than the maximum value provided in the table.
If \var{breakable} is 0 (default), a runtime error will be thrown. If it is non-zero, the pair and bond type will be queued for the \texttt{bond_breakage} mechanism. 
See section \ref{bondbreakage} for details.
